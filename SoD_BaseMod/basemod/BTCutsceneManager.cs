...
namespace SoD_BaseMod.basemod
{
	...
	public static void UpdateAvailableCutscenes(){
		if(ConfigHolder.config == null || ConfigHolder.config.availableCutscenes == null){
			ConfigHolder.LogMessage(LogType.Error, "could not find availableCutscenes in config!");
			return;
		}
		if(cutsceneDropdown == null){
			ConfigHolder.LogMessage(LogType.Error, "could not update available cutscenes, missing cutsceneDropdown from bundle.");
			return;
		}
	...
	private static void CutsceneLoadEvent(string inURL, RsResourceLoadEvent inLoadEvent, float inProgress, object inObject, object inUserData){
		...
		if(inObject == null || (bool)inUserData){
			ConfigHolder.LogMessage(LogType.Warning, "CutsceneLoadEvent yielded null object! (that's bad)");
			return;
		}
	...
	public void OnCutSceneDone(){
		if(currentCutsceneObject == null){
			ConfigHolder.LogMessage(LogType.Log, "Received CutSceneDone event, but no reason why, no cutscene was loaded");
			return;
		}
		
		ConfigHolder.LogMessage(LogType.Log, "Received CutSceneDone event for loaded cutscene");
		DeleteCurrentCutsceneObject();
	}
	}
}
